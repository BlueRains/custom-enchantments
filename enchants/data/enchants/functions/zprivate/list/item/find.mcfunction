...
# If an overwrite happens, a 1 will be stored in (global) CompFail Tools score. If values are the same, a 0 will be stored.
...
execute if score CompFail Tools matches 0 run function enchants:zprivate/list/item/exit

#if not continue iteration (until list is empty)
execute if score CompFail Tools matches 1 run function enchants:zprivate/list/item/continue